...
def do_scrapping(core_list):
    ...
    if os_name == 'Windows':
        ...
        executable = "bin/chromedriver_linux64/chromedriver"
    elif os_name == 'Darwin':
        executable = "bin/chromedriver_mac32/chromedriver"
    else:
    ...
    for core in core_list:
        url = f'https://artsandculture.google.com/asset/{core.get("museum_id")}/{core.get("asset_id")}'
        if '?' in url:
            url = url[:url.index('?')]
        driver.get(url)
        ...
        status = do_finally_changes(last_file, name_file)
        # print("status: ", status )
...
if __name__ == '__main__':
    root = tk.Tk()
    root.title('Google Art Downloader')
    root.resizable(0, 0)
